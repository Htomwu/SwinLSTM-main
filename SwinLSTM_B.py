...
#         self.offset_conv = nn.Conv2d(in_channels, deformable_groups * 2 * kernel_size ** 2,
#                                      kernel_size=kernel_size, stride=stride, padding=padding)
#         self.mask_conv = nn.Conv2d(in_channels, deformable_groups * kernel_size ** 2,
#                                    kernel_size=kernel_size, stride=stride, padding=padding)
#
...
#
#     def forward(self, x):
#         print(x.size())  # 查看输入张量的形状
#         B, L, C = x.size()  # 输入的形状 (B, L, C)
#
#         H = 180  # 高度
#         W = 360  # 宽度
#
#         # 由于 L = (H * W) / 4，确保 L 和 H * W / 4 匹配
#         assert L == (H * W) // 4, f"L should be {(H * W) // 4}, but got {L}"
#
#         # 将三维张量 (B, L, C) 转换为四维张量 (B, C, H, W)，将 L 视为时间步
#         # 这里应该将数据重塑为 (B, C, H, W) 作为输入
#         x = x.view(B, C, H//2, W//2)  # 确保我们将 L 映射到时间步，保持 H 和 W 不变
#
#         # Generate offsets and masks
#         offset = self.offset_conv(x)
#         mask = torch.sigmoid(self.mask_conv(x))  # Mask values between 0 and 1
#
#         # Reshape offsets and masks for deformable convolution
#         offset = offset.view(B, 2 * self.kernel_size[0] ** 2, H, W)
#         mask = mask.view(B, self.kernel_size[0] ** 2, H, W)
#
#         offset = offset.permute(0, 2, 3, 1).contiguous().view(B, H, W, -1)
#         mask = mask.view(B, H, W, -1)
#
#         # Apply deformable convolution
#         output = F.deform_conv2d(x, offset, mask, self.conv.weight, self.conv.bias, self.stride, self.padding,
#                                  self.dilation)
#
#         # 返回四维的输出结果
#         output = output.view(B, -1, self.out_channels)  # 将输出从四维转回三维
#         return output
#
#
...
#         super(SwinLSTMCell, self).__init__()
#
#         self.deformable_conv = DeformableConv2d(in_channels=dim, out_channels=dim, kernel_size=3, stride=1, padding=1)
#
#         self.Swin = SwinTransformerBlocks(dim=dim, input_resolution=input_resolution, depth=depth,
#                                           num_heads=num_heads, window_size=window_size, mlp_ratio=mlp_ratio,
#                                           qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
