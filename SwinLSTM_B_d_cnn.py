...
class DeformableConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, deformable_groups=1):
        ...
        self.offset_conv = nn.Conv1d(in_channels, deformable_groups * 2 * kernel_size,
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1).to(device)  # Change shape from (B, L, C) to (B, C, L)

        # Generate offsets and masks (these will be (B, deformable_groups * 2 * kernel_size, L))
        offset = self.offset_conv(x)
        mask = torch.sigmoid(self.mask_conv(x))  # Mask values between 0 and 1

        # Deformable Convolution logic
        # Offset reshaping
        offset = offset.view(B, 2 * self.kernel_size, L).to(device)  # For 1D: each location gets 2 offsets (horizontal shift)
        mask = mask.view(B, self.kernel_size, L).to(device)  # Each location gets a mask

        # Apply the deformable convolution manually: sampling based on the offsets
        output = self.deform_conv1d(x, offset, mask)

        # Reshape output back to (B, L, C) format
        output = output.view(B, L, self.out_channels).to(device)
        return output
    ...
    def deform_conv1d(self, x, offset, mask):
        ...
        """
        B, C, L = x.size()
        output = torch.zeros((B, L, self.out_channels), device=x.device)  # Prepare the output tensor

        # Expand mask to match x's shape
        mask = mask.view(B, self.kernel_size, L)  # Reshape mask to (B, kernel_size, L)

        # Broadcasting to avoid looping over each position in L
        mask_expanded = mask.unsqueeze(1).expand(B, C, self.kernel_size, L)  # Expand mask to (B, C, kernel_size, L)
        x_expanded = x.unsqueeze(2).expand(B, C, self.kernel_size, L)  # Expand x to (B, C, kernel_size, L)

        # Apply deformable convolution (computing sum over kernel_size dimension)
        output = (x_expanded * mask_expanded).sum(dim=2)  # sum over the kernel_size dimension
...
class SwinLSTMCell(nn.Module):
    ...
    def forward(self, xt, hidden_states):
        ...
        # Applying Deformable Convolution
        xt = self.deformable_conv(xt)
