...
class DeformableConv1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Offset reshaping
        offset = offset.view(B, 2 * self.kernel_size, L)  # For 1D: each location gets 2 offsets (horizontal shift)
    ...
    def deform_conv1d(self, x, offset, mask):
        ...
        offset_expanded = offset.unsqueeze(1).expand(B, C,2 * self.kernel_size, L)  # (B, C, kernel_size, L)
    ...
    def sample_from_offset(self, x, offset):
        """
        Sample values from the input tensor `x` based on the offset.
        This should be done with bilinear interpolation or nearest neighbor sampling.
        """
        B, C, K, L = x.size()

        # You need to implement the sampling logic that uses the offset to modify where the samples are taken
        # This could be bilinear interpolation or another method to sample from the `x` tensor.
        # For now, we just return `x` (you should replace this with real sampling logic).

        return x  # Replace with actual sampling code (e.g., using grid_sample or another method)
