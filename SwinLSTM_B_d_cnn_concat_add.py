...
class LocalGlobalFusionLayer(nn.Module):
    def __init__(self, local_dim, global_dim, fusion_dim):
        ...
        # 拼接后维度为 local_dim + global_dim
        self.concat_fc = nn.Linear(local_dim + global_dim, fusion_dim)

    def forward(self, local_features, global_features):
        # 在特征维度上进行拼接 (通常是 dim=-1)
        fusion_out = torch.cat([local_features, global_features], dim=-1)
        fusion_out = F.relu(self.concat_fc(fusion_out))
        return fusion_out
