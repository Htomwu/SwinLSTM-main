...
class SwinLSTMCell(nn.Module):
    ...
    def forward(self, xt, hidden_states):
        ...
        Ft = self.Swin(xt, hx)
        Ft = torch.clamp(Ft, min=-100, max = 100)
        # SLSTM gates
        gate = torch.sigmoid(Ft)
        # f_t = torch.exp(Ft)
        # i_t = torch.exp(Ft)  # Replace with exp for SLSTM
        # o_t = torch.sigmoid(Ft)
        z_t = torch.tanh(Ft)

        # Update cell and normalizer state
        c_t = gate * cx + gate * z_t
        c_t = torch.clamp(c_t, min=-1e3, max=1e3)
        # n_t = gate * nt + gate
        n_t = torch.clamp(gate * nt + gate, min=1e-6)
        # Compute hidden state
        h_hat = torch.clamp(c_t / (n_t + 1e-6),min =-100,max=100)
        # h_hat = torch.clamp(c_t / (n_t + 1e-6), min=-100, max=100)
        h_t = gate * h_hat
        h_t = torch.clamp(h_t, min=-100, max=100)
