...
class SwinSLSTMCell(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size, depth,
        ...
        super(SwinSLSTMCell, self).__init__()
...
#             hx = torch.zeros(B, L, C).to(xt.device)
#             cx = torch.zeros(B, L, C).to(xt.device)
#             nt = torch.ones(B, L, C).to(xt.device)  # Normalizer state
#         else:
#             hx, cx, nt = hidden_states
#
#         Ft = self.Swin(xt, hx)
#
#         # SLSTM gates
#         f_t = torch.sigmoid(self.forget_gate(Ft))
#         i_t = torch.exp(self.input_gate(Ft))  # Replace with exp for SLSTM
#         o_t = torch.sigmoid(self.output_gate(Ft))
#         z_t = torch.tanh(self.cell_input(Ft))
#
#         # Update cell and normalizer state
#         c_t = f_t * cx + i_t * z_t
#         n_t = torch.clamp(f_t * nt + i_t * z_t, min=1e-3)  # Prevent division by zero
#
#         # Compute hidden state
#         # h_hat = c_t / (n_t + 1e-6)
#         h_hat = torch.clamp(c_t / (n_t + 1e-6), min=-100, max=100)
#         h_t = o_t * h_hat
#
...
class DownSample(nn.Module):
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths_downsample, num_heads, window_size,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = SwinSLSTMCell(dim=int(embed_dim * 2 ** i_layer),
...
class UpSample(nn.Module):
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths_upsample, num_heads, window_size, mlp_ratio=4.,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = SwinSLSTMCell(dim=dimension, input_resolution=(resolution1, resolution2),
